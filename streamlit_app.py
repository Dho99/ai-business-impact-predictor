...
def main():
    ...
    # Market expectations
    st.sidebar.markdown("### 📊 Ekspektasi Pasar")
    
    # Estimated reviews based on business type and kecamatan
    base_reviews = {
        'Restaurant': 150, 'Cafe': 120, 'Fast food': 200, 'Bakery': 80,
        'Local Food': 100, 'Chinese Food': 90, 'Western Food': 110,
        'Korean Food': 85, 'Seafood': 95, 'Fine dining': 60
    }
    
    # Adjust based on kecamatan infrastructure
    kecamatan_multiplier = (kecamatan_data['malls'] + kecamatan_data['minimarkets']) / 20
    estimated_base = int(base_reviews.get(kategori_resto, 100) * max(0.5, kecamatan_multiplier))
    
    default_ulasan = getattr(st.session_state, 'jumlah_ulasan', estimated_base)
    jumlah_ulasan = st.sidebar.number_input(
        "Perkiraan Jumlah Review", 
        min_value=0, 
        value=default_ulasan,
        step=10,
        help=f"Perkiraan jumlah review yang akan didapat (rekomendasi: {estimated_base})"
    )
    ...
    if hasattr(st.session_state, 'prediction_made') and st.session_state.prediction_made:
        ...
        # Business recommendations in Indonesian
        st.markdown('<h3 class="section-header">💡 Rekomendasi Bisnis</h3>', unsafe_allow_html=True)
        
        if predicted_class == "Go":
            st.success(f"""
            **Rekomendasi untuk Kecamatan {st.session_state.kecamatan_data['name']}:**
            - ✅ Lokasi sangat prospektif, segera lakukan perencanaan bisnis
            - ✅ Amankan lokasi strategis secepat mungkin 
            - ✅ Fokus pada kualitas pelayanan untuk mempertahankan keunggulan
            - ✅ Pertimbangkan positioning premium sesuai kondisi area
            - ✅ Manfaatkan infrastruktur yang sudah baik di area ini
            """)
        elif predicted_class == "Consider":
            st.warning(f"""
            **Pertimbangan untuk Kecamatan {st.session_state.kecamatan_data['name']}:**
            - ⚠️ Lakukan riset pasar mendalam sebelum memutuskan
            - ⚠️ Analisis kompetitor di area sekitar dengan detail
            - ⚠️ Kembangkan unique value proposition yang kuat
            - ⚠️ Siapkan modal yang cukup untuk periode break-even lebih lama
            - ⚠️ Pertimbangkan strategi marketing yang intensif
            """)
        else:
            st.error(f"""
            **Mitigasi Risiko untuk Kecamatan {st.session_state.kecamatan_data['name']}:**
            - ❌ Pertimbangkan lokasi alternatif di kecamatan lain
            - ❌ Jika tetap lanjut, fokus pada optimasi biaya operasional
            - ❌ Kembangkan strategi diferensiasi yang sangat kuat
            - ❌ Pertimbangkan model franchise atau kemitraan
            - ❌ Siapkan investasi marketing yang besar untuk penetrasi pasar
            """)
        ...
        with tab1:
            ...
            3. **📊 Set Ekspektasi**: Masukkan perkiraan jumlah review dan target rating
            ...
            **Fitur Unggulan:**
            ...
            - ✅ **AI Analysis**: Model machine learning yang dilatih dengan data bisnis FnB real
            - ✅ **Rekomendasi Spesifik**: Saran bisnis yang disesuaikan dengan karakteristik lokasi
